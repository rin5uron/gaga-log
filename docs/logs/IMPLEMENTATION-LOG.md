...
## 2026年1月18日：記事テンプレート改善・パパラッチ・テレフォン記事の修正

### 概要
記事の目的と読者を明確にし、感想文ではなく解釈・考察を重視する構造に改善。パパラッチとテレフォンの記事を中心に、新しいテンプレートに沿って修正を実施。
...
**目的と読者を明確化**:
- 記事作成前に「この曲で何を伝えたいか」を明確にするセクションを追加
- 想定読者（初めて聴いた人／ファン）を意識した構成に変更
- 感想文ではなく「解釈・考察」を重視するスタンスを明文化
...
#### 2. 「この記事でわかること」セクションの改善

**変更内容**:
- 箇条書きで2〜3点までに制限（多すぎると焦点がぼける）
- SEOキーワードを含める
- 読者の疑問に直結する内容に
- 具体的＋問いかけ形式

**実装場所**: `ARTICLE_TEMPLATE.md`, `templates/template-song.md`
...
updatedDate: "YYYY-MM-DD"  # 更新日（構成・考察を見直した日）
```

**表示機能**:
- 記事ページ上部に控えめに表示（小さなテキスト、目立たせない）
- 構造化データ（JSON-LD）にも更新日を反映

**実装ファイル**:
- `lib/posts.ts`: Post型に`updatedDate`を追加
- `app/posts/[slug]/page.tsx`: 作成日・更新日の表示と構造化データの更新
...
**注意**: 無理に増やさず「考察の裏付け」として使う方針を明記

**実装場所**: `ARTICLE_TEMPLATE.md`にReferencesセクションを追加

#### 5. パパラッチ・テレフォン記事の修正

**対象記事**:
- `content/posts/paparazzi.md`
- `content/posts/telephone.md`

**修正内容**:
- 新しいテンプレート構成に沿って記事を再構成
- 感想文的な表現を解釈・考察に変更
- 「この記事でわかること」を明確化
- 参考情報セクションの追加（あれば）
...
1. `/ARTICLE_TEMPLATE.md` - 記事作成マニュアルを全面更新
2. `/templates/template-song.md` - テンプレートファイルを更新
3. `/lib/posts.ts` - Post型に`updatedDate`を追加
4. `/app/posts/[slug]/page.tsx` - 作成日・更新日の表示と構造化データの更新
5. `/content/posts/paparazzi.md` - テンプレートに沿って修正
6. `/content/posts/telephone.md` - テンプレートに沿って修正

### 効果

- ✅ 記事の目的と読者を明確化
- ✅ 解釈・考察重視の構造に統一
- ✅ 読む価値を最初に明示（「この記事でわかること」）
- ✅ 記事の更新履歴を可視化（作成日・更新日）
- ✅ 考察の裏付けを明確化（参考情報セクション）
